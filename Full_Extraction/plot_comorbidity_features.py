...
Produces 2 email-ready figures:
  1. comorbidity_summary.png — mean counts + % with >=1, all 4 systems
  2. poa_impact_combined.png — top 10 POA-affected categories per system (2x2)
"""
...
# ---------------------------------------------------------------------------
# Display labels for Elixhauser categories
# ---------------------------------------------------------------------------
AHRQ_LABELS = {
...
def get_prevalences(df, prefix, label_map, exclude_suffixes=("_vw_score", "_count", "_raf_score", "_sys_count")):
...
def main():
    ...
    print("\nFigure 1: Summary comparison...", flush=True)
    ...
    print(f"\nDone. Figures saved to {FIG_DIR}/", flush=True)
